firing = table2array(DA22022215191);
...
V_Raw = detrend(V_Raw); 
V_Raw = V_Raw - 60; % if use detrend function
...
% Set the AHP duration
disp('In order to preallocate the matrix for averaged spike,')
AHP_input = "please input how long you want to extend the AHP (Please input the interger and the unit is ms): \n";
AHP_length = input(AHP_input);
PrePeak_length = 20; % 20ms is the time duration before the peak

% Set the parameter for 'dV/dt vs Voltage' plot (Average_spikes_fun)
disp('Set the parameter for "dV/dt vs Voltage" plot')
...
[t_avg,avg_spike,avg_spike_dt,d_avg_spike_dt,avg_spike_pks,avg_spike_loc,avg_spike_neg_pks,avg_spike_neg_pks_3ms,avg_spike_neg_pks_20ms,avg_spike_neg_pks_150ms,avg_spike_neg_loc,avg_half_widths,time_spike,avg_spike_threshold] = Average_spikes_fun(V_Clean,CleanV_pos_loc,AHP_length,PrePeak_length,AHP_extend,PrePeak_extend,t,T,name);
