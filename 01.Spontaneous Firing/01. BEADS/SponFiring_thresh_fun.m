function [dddCleanV,t_diff3,dddCleanV_spike_loc,dddCleanV_1st_spike_p,CleanV_threshold] = SponFiring_thresh_fun(V_Clean,CleanV_pos_loc,t,T)
...
ref_point = CleanV_pos_loc - 1.4; % define ref_point at 1.4ms before each peak
...
[dddCleanV_spike_p,dddCleanV_spike_i] = arrayfun(@(col)findpeaks(dddCleanV_sub(:,col),'MinPeakHeight',800,'MinPeakProminence',400,'Npeaks',1),1:size(dddCleanV_sub,2),'UniformOutput',false); 
% 'MinPeakProminence' = 400 used for the coarse curve
...
dddCleanV_1st_spike_sub_i = cellfun(@(v)v(1),dddCleanV_spike_i); % location on "dddCleanV_sub" index 
dddCleanV_1st_spike_i = dddCleanV_1st_spike_sub_i + round(ref_point/T) - 1; % switch "dddCleanV_sub" index into "t" or "V_Clean" index; '-1': because when slicing dddCleanV_sub, the sub contains the index 'ref_point/T'
